...
class InMemoryHistoryManagerTest {
    ...
    void beforeEach() {
        ...
        Task task2 = new Task("Тест", "Проверить, что последняя задача, которую посмотрели, идет первой в списке", TaskStatus.NEW);
        taskManager.createTask(task2);
        taskManager.getTaskById(task2.getId());

        Task newTask = new Task("Задачи", "Проверить, что при удалении задачи, история очищается", TaskStatus.IN_PROGRESS);
        newTask.setId(1);
